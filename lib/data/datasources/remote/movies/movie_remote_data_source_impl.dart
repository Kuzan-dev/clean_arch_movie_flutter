...
class MovieRemoteDataSourceImpl implements MovieRemoteDataSource {
  ...
  Future<MovieListModel>getTopRatedMovies({required int page}) async {
    try {
      ...
      final model = MovieListModel.fromJson(response.data as Map<String, dynamic>);
      return model;
    } catch (_){
      rethrow;
    }
  ...
  Future<MovieDetailsModel>getMovieDetails({required int movieId}) async {
    try {
      final response = await _dioClient.get(UrlConstants.movieDetails.replaceAll('{movie_id}', movieId.toString()));

      final model = MovieDetailsModel.fromJson(response.data as Map<String, dynamic>);
      return model;
    } catch (_){
      rethrow;
    }
  ...
  @override
  Future<MovieListModel>getSimilarMovies({required int movieId, required int page}) async {
    try {
      final response = await _dioClient.get(UrlConstants.similarMovies.replaceAll('{movie_id}', movieId.toString()), queryParameters: {'page': page});
